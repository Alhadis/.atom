...
// Provide a more readable way of reading tokenised CSS scopes
Object.defineProperties(HTMLSpanElement.prototype, {
	scopeName: {
		configurable: true,
		get(){ return this.scopes.join("."); },
		set(to){ this.scopes = to; },
	},
	scopes: {
		configurable: true,
		get(){
			const results = [];
			for(const scope of this.classList)
				results.push(scope.startsWith("syntax--") ? scope.slice(8) : scope);
		...
		set(to){
			...
			// Store the original class list so we can recover it on error
			const from = this.className;
			try{
				const classes = [];
				for(let scope of to){
					if((scope = scope.trim()).startsWith("."))
						scope = scope.slice(0, 1);
					if(scope.startsWith("syntax--"))
						scope = scope.slice(8);
					classes.push(`syntax--${scope}`);
				}
				this.className = "";
...
// Alias `rootElement` to `documentElement`
...
// Differences (in seconds) between the Apple Epochs and the Unix epoch
Object.defineProperties(Date, {
	MAC_EPOCH_DIFF:    {value: 2082844800, enumerable: true}, // Mac HFS+ timestamp
	MAC_EPOCH_DIFF_CD: {value: 978307200,  enumerable: true}, // Cocoa Core Data timestamp
});
...
/**
 * Redefine {@link RegExp.prototype.compile} so it produces a more meaningful error
 * message if the calling object has been [frozen]{@link Object.freeze}.
 *
 * @param {String} source
 * @param {String} [flags]
 * @throws {TypeError}
 * @return {RegExp}
 */
const desc = Object.getOwnPropertyDescriptor(RegExp.prototype, "compile");
desc && desc?.configurable && Object.defineProperty(RegExp.prototype, "compile", {
	configurable: true,
	enumerable:   desc.enumerable,
	writable:     desc.writable,
	value: function compile(source, flags){
		if(Object.isFrozen(this))
			throw new TypeError("Cannot recompile frozen regexp");
		const fn = "function" === desc.get
			? desc.get.call(this)
			: desc.value;
		return fn.call(this, source, flags);
	},
});
