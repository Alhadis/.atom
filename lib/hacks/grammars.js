...
const {isDevToolsOpened} = require("./devtools.js");
const {Asar, loadGrammar, registerGrammar} = require("../utils/loaders.js");
const {asciify, findGrammar} = require("../utils/other.js");
const {OnigRegExp} = loadFromCore("oniguruma");
const Injections = loadFromCore("first-mate/lib/injections");
...
const {resolveScopeName, getRegex} = Pattern.prototype;
...
Pattern.prototype.getRegex = function(...args){
	lastPattern = {context: this, args};
	return getRegex.apply(this, args);
};
...
Scanner.prototype.createScanner = function(firstLine, position, anchorPosition){
	try{ return createScanner.call(this, firstLine, position, anchorPosition); }
		...
		console.error({context: this, error, args, lastPattern});
		throw error;
	}
};

Object.defineProperty(Injections.prototype, "isEmpty", {
	configurable: true,
	get(){
		return !(
			this.injections.length ||
			Object.keys(this.scanners).length
		);
	},
});
...
function loadForeignGrammars(pkg){
	...
	// Abort if this package doesn't appear to contain grammars written in unsupported formats
	if("language-not-mine" !== pkg.name && !(pkg.name || "").toLowerCase().endsWith(".tmbundle"))
		return false;
	const grammars = [];
	const dir = (
...
function loadVSCodeGrammars(pkg){
	if(foreigners.has(pkg))
		return foreigners.get(pkg);
	
	// Abort if this isn't a VSCode-specific package
	const {contributes = {}, engines = {}} = pkg.metadata;
	if(!engines.vscode || engines.atom)
		return false;

	// Abort if there're insufficient contribution points defined in manifest
	const {
		languages = [],
		grammars  = [],
		snippets  = [],
	} = contributes;
	if(languages.length + grammars.length < 1)
		return false;
	
	// Construct a hash-table of language contribution points mapped by ID
	const langsByID = {__proto__: null};
	for(const language of languages)
		langsByID[language.id] = language;
	
	// Start loading the package's grammars
	const results = [];
	for(const grammar of grammars){
		let {scopeName, path} = grammar;
		const src = loadGrammar(path = resolve(pkg.path, path), pkg, true);
		console.assert(scopeName === src.scopeName, "Gramamr scopeName differs to manifest");
		
		// Define or append list of associated filenames/filetypes
		const {extensions = [], filenames = []} = langsByID[grammar.language] || {};
		(src.fileTypes ||= []).push(...extensions.concat(filenames).map(type =>
			(type = String(type)).startsWith(".") ? type.slice(1) : type));
		
		results.push(registerGrammar(src, path, pkg));
	}
	
	foreigners.set(pkg, results);
	return results;
}
...
new Promise(resolve => {
	...
	const fn = pkg => loadVSCodeGrammars(pkg) || loadForeignGrammars(pkg);
