...
atom.packages.activatePackage("tree-view").then(({mainModule: {treeView}}) => {
	...
	treeView.onMouseDown = function(event){
		const selector = "li[is='tree-view-directory'].project-root.expanded:first-child";
		let projectEntry;
...
BracketMatcherView.prototype.isScopeCommentedOrString = function(...args){
	...
	|| args[0].some(scopeList => /^stri.*\.restructuredtext$/.test(scopeList))
...
// Give MagicPython a real grammar name
atom.packages.activatePackage("MagicPython").then(pkg => pkg.grammars.forEach(grammar =>
	grammar.name = grammar.name.replace(/^Magic\s*(?=\S)/i, "")));
...
atom.commands.onDidDispatch(({type}) => {
	...
	atom.workspace.emitter.once("did-stop-changing-active-pane-item", editor => {
		if(editor && atom.grammars.nullGrammar === editor.getGrammar())
...
const customTypes = {
	"source.c":     ["xbm"],
	"source.ini":   ["url"],
	"source.js":    ["cjs", "jsx", "psjs"],
	"source.json":  ["css.map", "js.map", "mjs.map", "nycrc", "c8rc", "hintrc", "ecrc"],
	"source.plist": ["glyphs", "proplist", "xccolortheme"],
	"source.ruby":  ["kick"],
	"source.shell": ["xinitrc", "xserverrc", "xsession"],
	"text.xml.plist": ["xccolortheme"],
};
...
const fixGrammars = {
	...
	// Filetypes deferred to heuristics; see “Re-examine…” handler near the EOF
...
// Don't query GitHub's APIs upon receiving a fatal error
Object.assign(loadFromCore("notifications/lib/user-utilities.js"), {
	async getLatestAtomData(){
		return {name: atom.appVersion.replace(/-.*$/, "")};
	},
	async getLatestPackageData(name){
		const pkg = atom.packages.getLoadedPackage(name);
		return {name, releases: {latest: pkg.metadata.version}};
	},
});
Object.assign(loadFromCore("notifications/lib/notification-issue.js").prototype, {
...
atom.notifications.onDidAddNotification(notice => {
	...
	// Hide that annoying issue-submission advice
	if("fatal" === notice.type){
		const {element} = atom.views.getView(notice);
		element.querySelector(".meta.item")?.remove();
	}
});
...
// Auto-assign JSON grammar to buffers reformatted by `user:format-json`
atom.commands.onDidDispatch(event => {
	if("user:format-json" === event.type){
		const editor = event.target.closest("atom-text-editor").getModel();
		const grammar = editor.getGrammar().scopeName;
		if("text.plain.null-grammar" === grammar || "text.plain" === grammar)
			getGrammar("source.json").then(json => editor.setGrammar(json));
	}
});


// Identify Adblock filters from uBlock Origin's conspicuous header
getGrammar("text.adblock").then(adblock => {
	const header = /^\[(?!\s*;)(?:(?:\s*;\s*)?(?:adblock(?:\s+plus)?|ublock(?:\s+origin)?|adguard)(?:\s+\d+(?:\.\d+)*)?)+\s*\]/i;
	atom.workspace.observeTextEditors(ed => {
		if("text.plain" !== ed.getGrammar().scopeName) return;
		if(header.test(ed.buffer.lineForRow(0)))
			ed.setGrammar(adblock);
	});
});


// Consult gitattributes(5) when opening “plain” text files
atom.packages.activatePackage("file-icons").then(({path}) => {
	const {FileSystem: AtomFS} = require(`${path}/node_modules/atom-fs`);
	assert(globalThis.AtomFS === AtomFS, "Duplicate AtomFS instances");
	const linguist = require(`${path}/lib/service/strategies/linguist-strategy.js`);
	
	atom.workspace.observeTextEditors(ed => {
		const path = ed.getPath();
		if(path && "text.plain" === ed.getGrammar().scopeName){
			const icon = linguist.matchIcon(AtomFS.get(path));
			if(!icon || !icon.scope) return;
			
			const grammars = atom.grammars.textmateRegistry.grammarsByScopeName;
			for(const scope in grammars){
				if(!icon.scope.test(scope)) continue;
				const grammar = grammars[scope];
				if(grammar.name && !grammar.source?.hideFromUser){
...
atom.workspace.observeTextEditors(ed => {
	ed.buffer.onWillSave(() => {
		if("source.regexp" === ed.getGrammar().scopeName) return; // Allow injection tests
...
wait(500).then(() => {
	for(const buffer of new Set([...atom.textEditors.editors].map(ed => ed.buffer))){
		...
		// If no filetypes were recognised, fallback on some heuristics 
		if(buffer.languageMode.grammar === atom.grammars.nullGrammar){
			if(!buffer.existsOnDisk) continue;
			const name = buffer.getBaseName();
			switch(name.toLowerCase().match(/\.[^.\s]+$/)[0]){
				case ".conf":
					if(buffer.findSync(/^\s*<\/?\w+(?=>|\s)[^<>\r\n]*>\s*$/m))
						buffer.grammar = "source.apache-config";
					else if(buffer.findSync(/^([[\]\s*a-zA-Z0-9_.-]+)\s*(=|:)/m))
						buffer.grammar = "source.ini";
					break;
			}
		}
	}
...
// Stop DevTools opening automatically at startup
atom.onWillThrowError(event => void event.preventDefault());
