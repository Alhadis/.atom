...
const {Range, Point, TextBuffer, TextEditor} = require("atom");
const {Cursor, Selection, DisplayMarker, getToken, getTokens} = require("../utils/buffer.js");
...
Object.defineProperties(TextBuffer.prototype, {
	existsOnDisk: {
		get(){
	...
	grammar: {
		get(){
			return this.languageMode.grammar;
		},
		set(to){
			to = !to ? atom.grammars.nullGrammar : "string" === typeof to ? findGrammar(to) : to;
			if(to !== this.languageMode.grammar)
				this.setLanguageMode(atom.grammars.languageModeForGrammarAndBuffer(to, this));
		},
	},
});

const editorSchema = atom.config.getSchema("editor");
editorSchema.properties.formatter = {
	type: "object",
	title: "Formatter",
	description: "A formatter command specification and its designated type.",
	properties: {
		command: {
			order: 1,
			title: "Command",
			type: "string",
			default: "user:unfuck-tabstops",
			description: String.raw `
				A command to reformat/beautify source-code, the exact
				form of which is dependent on the \`type\` field below.
			`.trim().replace(/\s+/g, " "),
		},
		type: {
			order: 2,
			...
			default: "Atom command",
			description: String.raw `
				Format of the command specifier string, which determines the appropriate syntax:
				* ‘Atom command’: The syntax is of a named workspace command (i.e., ‘namespace:command-name’).
				* ‘Global’: The string refers to a JavaScript global (i.e., ‘window.formatCode’), and is assumed to be a function that accepts input text as its first argument.
				* ‘External command’: The string is a shell-command that reads input either from stdin (if the first character of the command is ‘|’) or as the positional parameter ‘$1’.
			`.trim().replace(/^\s+/g, " ").replace(/‘|’/g, "`"),
		},
	},
};
atom.config.setSchema("editor", editorSchema);
atom.config.setRawScopedValue("editor.formatter", {
	command: "formatJSON",
	type: "Global",
}, null, ".source.json");
...
Object.defineProperties(TextEditor.prototype, {
	...
	getToken:  { value(...args){ return getToken(this, ...args); }},
	getTokens: {*value(...args){ yield* getTokens(this, ...args); }},
