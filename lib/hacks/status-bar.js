...
class CodepointView{
	...
	/**
	 * Retrieve the character at the given buffer position.
	 * @param {Point|Number|BigInt} pos
	 * @return {String}
	 * @internal
	 */
	charAt(pos){
		if("number" === typeof pos || "bigint" === typeof pos)
			pos = ed.buffer.positionForCharacterIndex(Number(pos));
		const char = ed.buffer.getCharacterAtPosition(pos);
		if(this.surrogateName(char)){
			const next = ed.buffer.getCharacterAtPosition(pos.translate([0, 1]));
			if(this.surrogateName(next))
				return char + next;
		}
		return char;
	}
	...
	/**
	 * Retrieve the type of surrogate-half the given character is, assuming it is one.
	 * @param {String} char
	 * @return {String}
	 * @internal
	 */
	surrogateName(char){
		...
		if(code < 0xDB80) return "High Surrogate";
		if(code < 0xDC00) return "High Private Use Surrogate";
	...
	update(){
		...
		this.updateSubscription = atom.views.updateDocument(() => {
			...
			this.code = eof ? -1 : this.charAt(pos).codePointAt(0);
		});
	}
}
