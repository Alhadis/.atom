...
const TextEditorElement = loadFromCore("../src/text-editor-element");
...
function getGrammarAtCursor(cursor){
	...
	for(const scope of cursor.getScopeDescriptor().scopes.reverse()){
...
/**
 * Locate the nth token matching a scope selector.
 *
 * @example <caption>Select first matching token</caption>
 *    const range = editor.getToken("comment.block.documentation");
 ...
 * @return {TokenMatch}
 */
function getToken(editor, selector, nth = 1){
	if(!atom.workspace.isTextEditor(editor)){
		[selector, nth] = [editor, selector];
		editor = atom.workspace.getActiveTextEditor();
	}
	let nthFound = 0;
	if(!isFinite(nth = Number(nth)))
		throw new RangeError("'nth' argument must be finite number");
	nth = Math.max(1, nth);
	for(const token of getTokens(editor, selector))
		if(++nthFound >= nth) return token;
	return null;
}


/**
 * Locate tokens that match the given scope-selector.
 *
 * @example <caption>Select the third token scoped with "invalid.illegal"</caption>
 *     const [, , token] = getTokens(editor, "invalid.illegal");
 *     editor.setSelectedBufferRange(token, {preserveFolds: true});
 *     editor.scrollToBufferPosition(token.start);
 *
 * @param {TextEditor} [editor]
 * @param {String} selector
 * @yields {TokenMatch}
 */
function *getTokens(editor, selector){
	if(!atom.workspace.isTextEditor(editor)){
		selector = editor;
		editor = atom.workspace.getActiveTextEditor();
	}
	...
	for(const line of tb.tokenizedLines){
		++row;
		...
		for(const token of line.tokens){
			...
			if(token.matchesScopeSelector(selector)){
				const range = new Range([row, column], [row, column + length]);
				const fullLine = editor.buffer.rangeForRow(row).isEqual(range);
				yield Object.freeze(Object.defineProperties({
					text:   value,
					scopes: token.scopes.slice(),
					range, index, fullLine,
					toString(){ return this.text; },
					[Symbol.toStringTag]: "Token",
				}, {
...
/**
 * A token retrieved by {@linkcode getTokens}.
 *
 * So-named to avoid confusion with the {@link Token} class
 * in Atom's core (the one used by {@link TokenizedBuffer}).
 *
 * @typedef {Object} TokenMatch
 * @property {String}   text - Text content of the matched token.
 * @property {String[]} scopes - Copy of the matched token's scopes array.
 * @property {Range}    range - Buffer range for the matched token.
 * @property {Number}   index - Index of token within parent {@link TokenizedLine}.
 * @property {Boolean} [fullLine] - If range extends to the full extent of the buffer line.
 */
...
function toPoint(...args){
	...
	let [value] = args;
	if(args.length > 1){
		const x = typeof args[0];
		const y = typeof args[1];
		if(x === y && ("number" === x || "bigint" === x))
	...
	const row = keys.find(x => String(x).toLowerCase().match(/^(?:0|x|row|line|lineno|linenumber)$/i));
	const col = keys.find(y => String(y).toLowerCase().match(/^(?:1|y|col|column|columnno|columnnumber)$/i));
	return clip(row && col
