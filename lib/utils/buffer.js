...
function toPoint(...args){
	...
	if(Symbol.iterator in value){
		...
		return value.length > 1
			? new Point(~~Math.max(0, value[0]), ~~Math.max(0, value[1]))
...
function toRange(...args){
	...
	if(value instanceof Point)     return new Range(value, value);
	if(value instanceof Selection) return value.getBufferRange();
	if(value instanceof Range)     return value;
	if(Symbol.iterator in value)   return new Range(...[...value].map(n => toPoint(n)));
