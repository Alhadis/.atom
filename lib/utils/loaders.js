...
function inced(clearCache = false, editor = null, ...args){
	...
	switch(scope){
		...
		case "source.paperspecs":
			let separator = null;
			if(/^\s*\S[^,]*(?:\s*,\s*\S[^,]*){3}\s*\n/.test(text))
				separator = /\s*,\s*/;
			else if(/^\s*\S+(?:\s+(?:\d+(?:\.\d+)?|\.\d+)){2}\s+[a-z]+\s*\n/i.test(text))
				separator = /\s+/;
			if(separator)
				return cache[key] = Object.fromEntries(text.split("\n").filter(Boolean).map(line => {
					let [name, width, height, unit] = line.trim().split(separator);
					name   = name.toLowerCase();
					width  = parseFloat(width);
					height = parseFloat(height);
					unit   = (unit || "pt").toLowerCase();
					return [name, {width, height, unit}];
				}));
			else{
				const name = text.replace(/^\s*#.*$\n?/gm, "").trim();
				return cache[key] = (name.match(/^[^#\s,][^,\n]*(?=\s*$)/m) || [])[0] ?? null;
			}
		
		case "text.plain":
		case "text.plain.null-grammar":
			if(!text.startsWith("bplist")) break;
			// Fall-through
		
		case "source.plist":
		case "text.xml.plist":
			return cache[key] = text.startsWith("bplist") && editor.buffer.file?.existsSync()
				? loadPlist(readFileSync(path), path)
				: loadPlist(text, path);
		...
		case "text.xml.xsl": {
			...
			"dfxml" === doc.rootElement.tagName && extendDFXML(doc, path);
...
function parseTSV(input){
	const NL = /\r?\n|\r|\x85|\u2028|\u2029/g;
	const [fields, ...records] = input.replace(/^\uFEFF|(?:\r?\n|\r)+$/g, "").split(NL).map(x => x.split("\t"));
	const results = [];
...
/**
 * Extend an {@link XMLDocument} with properties unique to
 * [DFXML]{@link https://github.com/dfxml-working-group/dfxml_schema/blob/HEAD/dfxml.xsd}.
 *
 * @param {XMLDocument} doc - A DFXML document, which is modified by reference.
 * @param {String} [path=null] - Location of the DFXML file on disk
 * @return {XMLDocument} A reference to the now-modified DFXML document.
 * @internal
 */
function extendDFXML(doc, path = null){
	if(!(doc instanceof XMLDocument)) throw new TypeError("Subject isn't an XMLDocument object");
	if(path && !fs.existsSync(path))  throw new TypeError(`Path argument points to non-existent file: ${path}`);

	const getElement = (node, name) => {
		const tags = node.getElementsByTagName(name);
		if(1 !== tags.length)
			throw new TypeError(`Should only have one <${name}>!`);
		return tags[0];
	};
	const getByteRuns = root => {
		const brun = getElement(root, "byte_runs");
		const runs = brun.getElementsByTagName("byte_run");
		const {children} = brun;
		if(runs.length < 1)                 throw new TypeError("Should have at least byte_run element!");
		if(runs.length !== children.length) throw new TypeError("byte_runs should contain only byte_run elements!");
		const byteRuns = [];
		for(const run of children){
			for(const attr of run.attributes)
				if(!["offset", "img_offset", "len"].includes(attr.name))
					throw new TypeError(`Unrecognised byte_run attribute: ${attr.name}`);
			byteRuns.push({
				offset:    parseInt(run.getAttribute("offset"),     10),
				imgOffset: parseInt(run.getAttribute("img_offset"), 10),
				len:       parseInt(run.getAttribute("len"),        10),
			});
		}
		return byteRuns;
	};

	// Carve report metadata
	const creator  = getElement(doc, "creator");
	const buildEnv = getElement(creator, "build_environment");
	const execEnv  = getElement(creator, "execution_environment");
	doc.creator = {
		package: getElement(creator, "package").textContent,
		version: getElement(creator, "version").textContent,
		buildEnv: {
			compiler: getElement(buildEnv, "compiler").textContent,
			libraries: Object.fromEntries([...buildEnv.getElementsByTagName("library")].map(el => {
				const name    = el.getAttribute("name");
				const version = el.getAttribute("version");
				return [name, "none" === version ? null : version];
			})),
		},
		execEnv: {
			osSysName: getElement(execEnv, "os_sysname").textContent,
			osRelease: getElement(execEnv, "os_release").textContent,
			osVersion: getElement(execEnv, "os_version").textContent,
			host:      getElement(execEnv, "host").textContent,
			uid:       parseInt(getElement(execEnv, "uid").textContent, 10),
			startTime: new Date(getElement(execEnv, "start_time").textContent),
		},
	};
	const fVer = parseFloat(doc.creator.version);
	if(doc.creator.version === fVer.toString())
		doc.creator.version = fVer;

	const src = getElement(doc, "source");
	doc.source = {
		imageFilename: getElement(src, "image_filename").textContent,
		sectorSize:    parseInt(getElement(src, "sectorsize").textContent, 10),
		imageSize:     parseInt(getElement(src, "image_size").textContent, 10),
		volume:        {byteRuns: getByteRuns(getElement(src, "volume"))},
	};

	// Build file-map
	const files = doc.files = new Map();
	for(const el of doc.getElementsByTagName("fileobject")){
		for(const child of el.children)
			if(!["filename", "filesize", "byte_runs"].includes(child.tagName))
				throw new TypeError(`Unrecognised element name: ${child.tagName}`);
		const name = getElement(el, "filename").textContent;
		const size = parseInt(getElement(el, "filesize").textContent, 10);
		if(files.has(name))
			throw new TypeError(`Duplicate filename: ${name}`);
		const byteRuns = getByteRuns(el);
		const exists   = path && fs.existsSync(join(dirname(path), name));
		files.set(name, {name, size, exists, byteRuns});
	}
