 ...
 * @param {Boolean} [compileOnly=false] - Return a plain POJO
 * @return {Grammar|Object}
 */
function loadGrammar(path, compileOnly = false){
	let props;
	
	// XML property list
	if(/\.(tmLanguage|plist)$/i.test(path))
		props = loadPlist(readFileSync(path, "utf8"));
...
function loadPlist(input, path = ""){
	...
	return stdout ? Plist.parse(stdout) : null;
}
