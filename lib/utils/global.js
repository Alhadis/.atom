...
const {buildRegExp, getProperties} = require("./other.js");
...
Object.assign(global, {
	...
	inced(...args){
		...
		// HACK: Workaround for Atom's shitty ESM support
		else if(/\/Labs\/Utils\/lib\/[^/]+\.mjs$/i.test(editor.getPath())){
			let fns = [];
			let src = editor.getText()
				.replace(/^export\s+function\s+(\w+)/mg, (_, n) => (fns.push(n), `function ${n}`))
				.replace(/\bimport\(/g, "require(");
			fns = fns.map(x => `module.exports.${x} = ${x};`).join("\n");
			src = `return (function(module){\n${src}\n${fns}\nreturn module.exports})({exports: {}});`;
