...
/**
 * Strip trailing ID3 data from a WAV file.
 * @param {String} path
 * @return {Boolean} Whether the metadata removal was successful or not.
 * @public
 */
function stripWAVMetadata(path){
	let index, data = readFileSync(path);
	if("RIFF" === data.asciiSlice(0, 4) && ~(index = data.lastIndexOf("id3"))){
		const id3Size = data.length - index;
		const oldSize = data.readUInt32LE(4);
		const newSize = oldSize - id3Size;
		data = data.slice(0, index);
		deepStrictEqual(newSize, data.length - 8);
