...
const {dc, bc, round, getPrecision, tsvTable, wait} = require("../utils/other.js");
const {hasSelectedText, mutate} = require("../utils/buffer.js");
const {error, key, pipe, prompt, getEditor} = require("../utils/commands.js");
const {loadFromCore} = require("../utils/loaders.js");
const KeyPathHelpers = loadFromCore("key-path-helpers");
const EDITOR_PANES = "atom-text-editor:not([mini])";
const MATCH_NUMBER = /-?(?:\d+(?:\.\d*)?|\.\d+)/g;
...
atom.commands.add(EDITOR_PANES, "user:unfuck-tabstops", () => {
...
// Exclude trailing whitespace from current selection(s)
atom.commands.add(EDITOR_PANES, {
	"editor:select-all-except-trailing-whitespace"(event){
		const editor = event.currentTarget;
		editor.getModel().selectAll();
		return atom.commands.dispatch(editor, "editor:unselect-trailing-whitespace");
	},
	"editor:unselect-trailing-whitespace"(event){
		const editor = event?.currentTarget?.getModel() || this || atom.workspace.getActiveTextEditor();
		for(const selection of editor.getSelectionsOrderedByBufferPosition().filter(sel => !sel.isEmpty())){
			const {length: whitespace} = (selection.getText().match(/\s+$/) || [""])[0];
			if(whitespace && whitespace < editor.buffer.getLength()){
				const head = selection.getHeadBufferPosition();
				const tail = selection.getTailBufferPosition();
				const last = Math.max(0, Math.max(
					editor.buffer.characterIndexForPosition(head),
					editor.buffer.characterIndexForPosition(tail),
				) - whitespace);
				selection.setBufferRange([
					selection.isReversed() ? head : tail,
					editor.buffer.positionForCharacterIndex(last),
				], {autoscroll: false, preserveFolds: true});
			}
		}
	},
});
...
// Round off fractional values inside selection(s)
for(const method of ["round", "ceil", "floor"]){
	const fn = Math[method].bind(Math);
	atom.commands.add(EDITOR_PANES, `math:${method}`, () => {
		const editor = atom.workspace.getActiveTextEditor();
		mutate(editor, text => text.replace(MATCH_NUMBER, n => fn(parseFloat(n))));
	});
}


// Apply basic arithmetic operations to selected numbers
const mathOps = {
	add:      {verb: "Increase", fn: (a, b) => a + b},
	subtract: {verb: "Decrease", fn: (a, b) => a - b},
	multiply: {verb: "Multiply", fn: (a, b) => a * b},
	divide:   {verb: "Divide",   fn: (a, b) => a / b},
};
for(const op in mathOps){
	const {verb, fn} = mathOps[op];
	atom.commands.add(EDITOR_PANES, `math:${op}`, () => {
		const editor = atom.workspace.getActiveTextEditor();
		prompt(`${verb} selected number(s) by…`).then(value => {
			if(!isFinite(value = parseFloat(value))) return;
			mutate(editor, text => text.replace(MATCH_NUMBER, n => fn(parseFloat(n), value)));
		});
	});
}


// Evaluate an expression using arbitrary-precision arithmetic
for(const fn of [bc, dc])
	atom.commands.add(EDITOR_PANES, `math:eval-${fn.name}`, () =>
		mutate(atom.workspace.getActiveTextEditor(), text => fn(text)));
...
// Insert modification date of file, then write to disk while preserving timestamps
atom.commands.add(EDITOR_PANES, "editor:insert-modify-date", async () => {
	const editor  = atom.workspace.getActiveTextEditor();
	const path    = editor.getPath();
	if(!editor.buffer.file)  return error("File not saved to disk yet");
	if(!fs.existsSync(path)) return error("File missing or deleted");
	const {mtime} = fs.statSync(path);
	const replace = Intl.DateTimeFormat("en-AU", {
		dateStyle: "long",
		timeStyle: "long",
		hour12:    false,
		hourCycle: "h23",
		timeZone:  "Australia/Melbourne",
	}).format(mtime).replace(/, */, " ");
	editor.getLastSelection().insertText(replace, {select: true});
	await editor.save();
	await wait(100);
...
// Replace all occurrences of “%{TIMESTAMP}” in the editor's text before saving to disk
atom.commands.add(EDITOR_PANES, "editor:replace-timestamps-and-save", async () => {
	const editor = atom.workspace.getActiveTextEditor();
	const date = new Date();
	mutate(editor, text => text.replaceAll("%{TIMESTAMP}", date.toISOString()));
	await editor.save();
	await wait(100);
	fs.utimesSync(editor.getPath(), date, date);
});
...
// Add command wrappers for JavaScript's native string transformation functions
const methods = {
	"editor:decode-uri":           window.decodeURI,
	"editor:encode-uri":           window.encodeURI,
	"editor:decode-uri-component": window.decodeURIComponent,
	"editor:encode-uri-component": window.encodeURIComponent,
	"editor:escape":               window.escape,
	"editor:unescape":             window.unescape,
	"editor:atob":                 window.atob,
	"editor:btoa":                 window.btoa,
};
for(const commandName in methods){
	const fn = methods[commandName];
	atom.commands.add("atom-text-editor", commandName, event =>
		mutate(event.currentTarget.getModel(), fn));
}


// Reformat/beautify text
atom.commands.add(EDITOR_PANES, "editor:format", event => {
	const editor = getEditor(event);
	const config = getEditorSettings(editor);
	const {command, type} = config.formatter;
	switch(type){
		case "Atom command":
			if("editor:format" !== command){
				console.log(`Dispatching '${command}'`);
				atom.commands.dispatch(editor.element, command);
			}
			break;
		case "Global":
			const fn = KeyPathHelpers.getValueAtKeyPath(globalThis, command);
			if("function" === typeof fn)
				mutate(editor, text => fn(text, config));
			break;
		case "External command":
			if(command.startsWith("|"))
				mutate(editor, text => execSync(command.slice(1), {
					input: text,
					stdio: ["pipe", "pipe", "inherit"],
					encoding: "utf8",
					windowsHide: true,
				}));
			else mutate(editor, text =>
				execFileSync("/bin/sh", ["-c", command, "--", text], {
					encoding: "utf8",
					stdio: ["ignore", "pipe", "inherit"],
					shell: false, // This IS a shell
					windowsHide: true,
				}));
			break;
	}
...
/**
 * Retrieve all editor settings that apply to an editor's filetype.
 *
 * @internal
 * @todo Merge with/replace {@linkcode at|at()}?
 * @param {TextEditor} editor
 * @return {Object}
 *    A null-prototype object populated with `editor` configuration properties.
 *    Settings scoped to higher specificity take precedence over lower ones;
 *    i.e., `.source.js` editor settings override `*` editor settings.
 */
function getEditorSettings(editor){
	const scope   = editor.getRootScopeDescriptor();
	const configs = atom.config.getAll("editor", {scope});
	const merged  = {__proto__: null};
	while(configs.length)
		Object.assign(merged, configs.pop().value);
	return merged;
}


// Reformat JSON
function formatJSON(text, config){
...
// Reformat SVG paths exported by Adobe Illustrator
atom.commands.add(EDITOR_PANES, "user:format-svg", event =>
	mutate(event.currentTarget.getModel(), text => text.replace(
		/^([ \t]*)(<path(?=\s)[^>]*?\s+d=")([^"]+)"/gm,
		(_, indent = "", start = "", value = "") => {
			const singleLine = !value.includes("\n");
			value = value
				.replace(
					/^([a-zA-Z])([-, \t0-9.]*$(?:\n\1[-, \t0-9.]*$)+)/gm,
					(_, cmd, guts) => cmd + guts.replaceAll("\n" + cmd, " ").trim())
				.replace(/, *-/g, "-")
				.replace(/[\s,]+/g, " ")
				.replace(/([A-Za-z])[ \t]+/g, "$1")
				.replace(/[ \t]+([A-Za-z])/g, "$1")
...
atom.commands.add(EDITOR_PANES, "editor:go-to-offset", event => {
	...
	prompt("Go to byte:", ...args).then(response => {
		...
		if("-+".includes(response[0]))
			response = editor.offset + +response;
...
function bumpSelectedNumbers(by = 1, editor = null){
	...
	return editor.mutateSelectedText(selection => {
		const text = selection.getText().replace(MATCH_NUMBER, n => {
			n = parseFloat(n);
...
// Apply annoying effects to text using uniwank(1)
atom.commands.add(EDITOR_PANES, Object.fromEntries(Object.entries({
	n:   "remove",
	b:   "bold",
	i:   "italic",
	B:   "bold-italic",
...
// Convert unsafe filename characters into Unicode lookalikes
...
atom.commands.add(EDITOR_PANES, "editor:filesafe", event =>
	mutate(getEditor(event), filesafe));
