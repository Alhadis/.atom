...
atom.commands.add(EDITOR_PANES, "editor:expand", event => {
	...
	const expansions = editor.getCursors().map(cursor => {
		...
		|| /(?:&#|[%\\](?!...;))x([A-F0-9]{2});?$/ig.test(textBefore)){
			...
			return {range, char: String.fromCharCode(parseInt(lastParen, 16))};
		}
		...
		// Codepoint, octal
			...
			const {leftContext, lastParen} = RegExp;
			range.start.column = leftContext.length;
			return {range, char: String.fromCharCode(parseInt(RegExp.$1, 8))};
		}
		...
		// Caret notation
			...
			range.start.column = RegExp.leftContext.length;
			const char = String.fromCharCode(RegExp.lastMatch.charCodeAt(1) - 64);
			return {range, char};
		}
	...
	return expansions.length
		? editor.transact(100, () => expansions.map(({range, char}) =>
