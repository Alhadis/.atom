...
atom.commands.add("atom-text-editor", {
	...
	"user:curly-bracket": key("{", state => {
		...
		if(scopes.some(scope => /^\.source\.coffee$/)
		&& !/^\]/.test(state.textAfter)
		&&  /},$/.test(textBefore))
			return ["{"];
	}),
});
...
atom.commands.add(EDITOR_PANES, "editor:octal-dump-to-binary", event => {
	mutate(event.currentTarget.getModel(), text => {
		text = text.replace(/^[0-7]{7}\s+/gm, "").trim();
		return String.fromCharCode(...text.split(/\s+/g).map(byte => parseInt(byte, 8)));
	});
});
...
atom.commands.add(EDITOR_PANES, "editor:go-to-offset", () =>
	prompt("Enter a number:").then(response => {
		if(null == response || Number.isNaN(+response)) return;
		const editor = atom.workspace.getActiveTextEditor();
		const point = editor.buffer.positionForCharacterIndex(+response);
		editor.getLastCursor().setBufferPosition(point, {autoscroll: true});
	}));
