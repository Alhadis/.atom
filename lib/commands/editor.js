...
atom.commands.add("atom-text-editor", "user:copy", event =>
	hasSelectedText(atom.workspace.getActiveTextEditor()) && event.abortKeyBinding());
...
atom.commands.add(EDITOR_PANES, "user:codepoint-to-char", event => {
	...
	if(hasSelectedText(editor))
		return event.originalEvent && "Tab" === event.originalEvent.key
			...
			: mutate(editor, text => text.replace(/U\+([0-9A-Fa-f]{4,})/g, (_, n) =>
	...
	const cursors = editor.getCursors().map(cursor => {
		...
		if(/U\+([0-9A-Fa-f]{4,})$/g.test(textBefore)){
