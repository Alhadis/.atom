...
const commands = {
	...
	"user:unfuck-tabstops" (){
		...
		if(!editor) return;
		const squashedLines = editor.getText().match(/^\x20{2}/mg);
		
		// Seriously, fuck this tab-stop width.
		if(squashedLines.length > 1){
			editor.setSoftTabs(true);
			editor.setTabLength(2).then(() => {
				atom.commands.dispatch(editor.editorElement, "whitespace:convert-spaces-to-tabs");
			});
		}
	},
