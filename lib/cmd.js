...
const commands = {
	...
	/* Toggle editor's point-size between 24px (magnified) and normal size */
	...
	"atom-text-editor user:js-template-wrap" (){
	...
	/* JSDoc: Insert *-prefixed newline when hitting enter */
	"atom-text-editor user:js-doc-newline" (event){
		const editor = atom.workspace.getActiveTextEditor();
		const selection = editor.getLastSelection();
		const {scopes} = selection.cursor.getScopeDescriptor();
		
		if(-1 !== scopes.indexOf("comment.block.documentation.js"))
			editor.transact(10, () => {
	...
	"user:toggle-trendy": () => global.trendy = !global.trendy,
